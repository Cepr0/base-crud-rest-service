...
public interface UserMapper extends BaseMapper<Long, User, UserCreateRequest, UserUpdateRequest, UserResponse> {
	...
	default <ID extends Serializable, T extends BaseEntity<ID>> Set<T> toElements(@NonNull Collection<ID> ids, @NonNull BaseRepo<ID, T> repo) {
		return ids.stream()
				.map(repo::getOne)
				.collect(Collectors.toSet());
	}

	default <ID extends Serializable, T extends BaseEntity<ID>> Set<ID> toIds(@NonNull Collection<T> elements) {
